...
cap = cv2.VideoCapture(r"D:\project\video/for_Elise.mp4")
width = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))   # float
height = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT)) # float
print(width, height)
lastFrame = cv2.imread(r"D:\project\image/find4.jpg")
lastFrame = cv2.cvtColor(lastFrame, cv2.COLOR_BGR2GRAY)

"""""
lastFrame = (np.log(lastFrame+1)/(np.log(1+np.max(lastFrame))))*255
# Specify the data type
lastFrame = np.array(lastFrame,dtype=np.uint8)
# Display the image
"""""
...
def key_detection(img):
    ...
    cv2.line(edges, (0,0), (1920,0), (255,255,255), 35)
    cv2.line(edges, (0,508), (1920,508), (255,255,255),35)
    cv2.imshow('edges', edges)
    dilation = cv2.dilate(edges, kernel1, iterations = 1)#膨脹
    erosion = cv2.erode(dilation,kernel2,iterations = 1)#侵蝕
    ...
    num = 0
    letters = [ "ds,", "e,", "f,", "fs,", "g,", "gs,", "a,", "as,", "b,", "c", "cs", "d", "ds", "e", "f", "fs", "g", "gs", "a",
                "as", "b", "c'", "cs'", "d'", "ds'", "e'", "f'", "fs'", "g'", "gs'", "a'", "as'", "b'" ]
    label = []
    for box in key:
...
def findContours(img,ori):
    ...
    press=[]
    for c in contours:
        if (60000> cv2.contourArea(c) > 9000):
...
while True:
    ...
    detect = cv2.imread(r"D:\project\image/find4.jpg")
    key_img,coor,state,keyy=key_detection(detect) 
    cv2.imshow("key", key_img) 
    
    linkkk=coordinate(coor)
    cv2.line(frame, (0,0), (1920,0), (255,255,255), 5)
    cv2.line(frame, (0,508), (1920,508), (255,255,255),5)
...
for i in range(len(final2)):
    ...
    if final2[i][1]>=12:
        ...
        final2[i][1]=4
    elif 1.5<=final2[i][1]<3:
        final2[i][1]=2
    elif 0<=final2[i][1]<1.5:
        final2[i][1]=1
